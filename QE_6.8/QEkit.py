...
Getband_fun['fermi']    = -6.687886848627055e-2 # au, grep fermi pw_band.xml
...
def Getband(logfile, fermi, K_lable, K_coord):
	...
	with open(logfile, 'r') as f:
		...
		for line in data:
			...
			if 'bands (ev):' in line:
				tmp = data.index(line)
				lines = data[tmp+2].split()
				k_energy.append([float(i) for i in lines])
	...
	with open('BAND.dat', 'w+') as f:
		...
		for i in range(len(k_energy[0])): # bands
			...
			if i%2 == 1: # negative sequence, kpoints
				for ii in range(len(k_energy)-1,0,-1):
					f.writelines('    ' + '%0.4f' %k_distance[ii] + '    ' + '%0.4f' %k_energy[ii, i]+'\n')
				f.writelines('\n')
