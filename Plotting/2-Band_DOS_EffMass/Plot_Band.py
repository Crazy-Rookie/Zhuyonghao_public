...
'''
    ...
    there is a similar script: 'https://github.com/QijingZheng/pyband'
...
'''
Plot_band_fun                    = {}
Plot_band_fun['TorF']            = 'T' # band plot? T or F
Plot_band_fun['PlotMod']         = 'S' #'C'->colorbar 'B'->bubble 'S'->simple band type
#PlotMod=B 
Plot_band_fun['bubble_factor']   = 200 #band_p0*bubble_factor for bubble mode
#PlotMod=C
Plot_band_fun['color_bar_label'] = ['Bot','Top'] #[blue,red]
Plot_band_fun['size_sca']        = 20 #int
#General Settings
Plot_band_fun['show_']           = 'T' #plot? T or F
Plot_band_fun['save_']           = 'F' #save? T or F
Plot_band_fun['lw_']              = 2 #line width
Plot_band_fun['y_range']         = [-2,2] #[int,int]
Plot_band_fun['file_P']          = ['WS2.dat','MoS2.dat'] #[red,blue]
...
############################################################
Plot_dos_fun                     = {}
Plot_dos_fun['TorF']             = 'F' #band dos? T or F
Plot_dos_fun['show_']            = 'T' #plot? T or F
Plot_dos_fun['save_']            = 'F' #save? T or F
Plot_dos_fun['Tdos_show']        = 'F' #show Tdos? T or F
Plot_dos_fun['y_range']          = [0,8] #y range
Plot_dos_fun['y_step']           = 2 #y step
Plot_dos_fun['x_range']          = [-3,3] #x range
Plot_dos_fun['x_step']           = 1 #x step
...
def PlotBand(y_range=[-3,3],show_='Y',size_sca=20,save_='Y',
    ...
    if PlotMod == 'C':##color bar type
        ...
        #x and y range
        plt.xlim([min(x_h),max(x_h)]);plt.ylim(y_range)
        #y label
        plt.ylabel("Energy (eV)",size=20)
        #x and y ticks(step=1)
        plt.xticks(x_h,high_kpt,size=15)
        plt.yticks([i for i in range(min(y_range),max(y_range)+1)],
            [i for i in range(min(y_range),max(y_range)+1)],size=15)
        ...
        #save .svg 
        if save_ == 'T':
            plt.savefig('band_color.svg',format='svg')
        if show_ == 'T':
            plt.show()

    if PlotMod == 'B':##bubble type
        ##read data
        band_B,band_P0,x_h,band_p0,band_p1 = readBand(file_B=file_B,file_P=file_P,file_K=file_K)
        ##High Kpt of TMDs Materials(['Γ','M','K','Γ']), set highkpt for other materials.
        high_kpt = highkpt
        ...
        plt.yticks([i for i in range(min(y_range),max(y_range)+1)],
            [i for i in range(min(y_range),max(y_range)+1)],size=20)
        #high kpoint and fermi line
        x_h_ = x_h[1:-1]
        for i in x_h_:
            plt.axvline(x=i,c="black",lw=1.5)
        plt.axhline(y=0,ls='--',c="black",lw=1,zorder=1)
        ##band bubble
        band_bubble = plt.scatter(x,y,s=bubble_factor*band_p0,color='r',alpha=0.55,linewidth=0)
        plt.scatter(x,y,s=bubble_factor*band_p1,color='b',alpha=0.55,linewidth=0)
        #save .svg 
        if save_ == 'T':
            plt.savefig('band_bubble.svg',format='svg')
        if show_ == 'T':
            plt.show()
  
    if PlotMod == 'S':##simple band type
        band_B = np.loadtxt(file_B)
        kl_file = open(file_K,'r');x_h = []
        for line in kl_file:
            lines = line.split()
        ...
        band_line = plt.plot(x,y,c='black',zorder=0,lw=lw_)
        #x and y range
        plt.xlim([min(x_h),max(x_h)]);plt.ylim(y_range)
        #y label
        plt.ylabel("Energy (eV)",size=20)
        ...
        #save .svg
        if save_ == 'T':
            plt.savefig('band_simple_band.svg',format='svg')
        if show_ == 'T':
            plt.show()

def PlotDos(y_range=[0,3],y_step=1,x_range=[0,8],x_step=2,
    file_T='TDOS.dat',file_PD=['dw.dat','up.dat'],
    lw_=2,legend_=[],Tdos_show='T',color_PD=[], show_='T', save_='F'):
    ...
    #x and y ticks
    x_ = [min(x_range)+i*x_step for i in range(int((max(x_range)-min(x_range))/x_step)+1)]
    y_ = [min(y_range)+i*y_step for i in range(int((max(y_range)-min(y_range))/y_step)+1)]
    ...
    #save .svg 
    if save_ == 'T':
        plt.savefig('dos.svg',format='svg')
    if show_ == 'T':
        plt.show()

##Plot_band
if Plot_band_fun['TorF'] == 'T':
...
##Plot_dos
if Plot_dos_fun['TorF'] == 'T':
    PlotDos(y_range=Plot_dos_fun['y_range'], y_step=Plot_dos_fun['y_step'], x_range=Plot_dos_fun['x_range'],
        x_step=Plot_dos_fun['x_step'], file_T=Plot_dos_fun['file_T'], file_PD=Plot_dos_fun['file_PD'],
